...
<div class="wrap">
  ...
  <div class="tabs">
    ...
    <div class="tab" data-tab="signs">Road Signs &amp; Markings<span class="tcount">{nsigns} questions</span></div>
    <div class="tab" data-tab="rules">Traffic Rules &amp; Safety<span class="tcount">{nrules} questions</span></div>
  ...
  <div id="quiz">
    <div class="card">
      <div class="meta">
        <span class="badge" id="tabBadge">Full DKT</span>
        <span id="qProg"></span>
      </div>
      <div id="qHolder"></div>
      <div id="opts"></div>
  ...
  <p class="foot src">Questions are Australia DKT-style practice material (public domain style). Informational only, not affiliated with any state road authority or government agency.</p>
...
if __name__ == "__main__":
    ...
    spec = importlib.util.spec_from_file_location("audata", os.path.join(ROOT, "_au_data.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    au = mod.AU
    with open(os.path.join(OUT, "index.html"), "w") as f:
        f.write(page_html(au))
    nfull = sum(len(c["questions"]) for c in au["categories"])
    print("wrote au/index.html | full=%d signs=%d rules=%d alcohol=%d" % (
        nfull,
        len(au["categories"][0]["questions"]),
        len(au["categories"][1]["questions"]),
        len(au["categories"][2]["questions"]),
    ))
